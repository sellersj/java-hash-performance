...
public class Application {
     ...
     * @param file to generate a hash for
     */
    public static void checkFileBuffered(File file) {
        long t1 = System.currentTimeMillis();
        String hex = null;
        try {
            MessageDigest md = MessageDigest.getInstance("SHA-256");
            hex = checksumWithBufferedInput(file.getAbsolutePath(), md);
        ...
        long t2 = System.currentTimeMillis();
        printTime("It took %s milliseconds to create a checksum using buffered I/O which was " + hex, t1, t2);
    }
    ...
    // from NistDataMirror
    private static String checksumWithBufferedInput(String filepath, MessageDigest md) throws IOException {

        // file hashing with DigestInputStream
        try (
            DigestInputStream dis = new DigestInputStream(new BufferedInputStream(new FileInputStream(filepath)), md)) {
            while (dis.read() != -1) {
                ; // empty loop to clear the data
            }
            md = dis.getMessageDigest();
